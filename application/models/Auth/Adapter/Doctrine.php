...
class Dol_Model_Auth_Adapter_Doctrine implements Zend_Auth_Adapter_Interface {
    ...
    public function __construct($username , $password, $entityManager)
    {
    ...
    public function authenticate()
    {
        try
        {
            $this->user = Dol_Model_Entity_User::authenticate($this->username, $this->password, $this->_em);
        }
        catch (Exception $e)
        {
            if ($e->getMessage() == Dol_Model_Entity_User::WRONG_PW)
                return $this->result(Zend_Auth_Result::FAILURE_CREDENTIAL_INVALID, self::BAD_PW_MESSAGE);
            if ($e->getMessage() == Dol_Model_Entity_User::NOT_FOUND)
